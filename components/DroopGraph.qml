...
Item {
    ...
    property real _guidelineIntersectionRatio: (yAxisReferenceValue - yAxisLowerReferenceValue)
    ...
    // located. This is the blue dot and defines the active/reactive
    // power values (Hertz or voltage and percent
    property real _indicatorRatio: xAxisOperationValue/xAxisReferenceValue - 0.5
    ...
    QuantityLabel {
        ...
        property real _yCentre: (_guidelineIntersection.y + _indicator.y)/2
        property real _yAdjustment: Math.abs(_yCentre) > height/2
                                    ? height/2
                                    : height/2
                                       - Math.abs(_guidelineIntersection.y - _indicator.y)/2

        anchors.left: parent.left
        y: _yCentre + _yAdjustment * Math.sign(_guidelineIntersection.y - _indicator.y)
            - height/2
    ...
    QuantityLabel {
        ...
        anchors.left: parent.left
        y: yAxisReferenceQuantityLabel._yCentre - yAxisReferenceQuantityLabel._yAdjustment * Math.sign(_guidelineIntersection.y - _indicator.y)
            - height/2
        width: Theme.geometry_droopGraph_yAxis_quantityLabel_width

        value: yAxisReferenceValue.value + (xAxisReferenceValue - xAxisOperationValue) * droop
        unit: VenusOS.Units_Percentage
        precision: 0
    ...
    QuantityLabel {
        ...
        // This calculates the centre value between the two guidelines
        // and then sets a +/- adjustment to be applied to the two labels
        property real _xCentre: (_guidelineIntersection.x + _indicator.x)/2
        property real _xAdjustment: Math.abs(_guidelineIntersection.x - _indicator.x) + 10 > width/2
                                    ? width/2 + 10
                                    : width/2 + 10 // Padding between labels
                                       - Math.abs(_guidelineIntersection.x - _indicator.x)/2

        anchors.bottom: parent.bottom
        x: _xCentre + _xAdjustment * Math.sign(_guidelineIntersection.x - _indicator.x)
            - width/2
    ...
    QuantityLabel {
        ...
        anchors.bottom: parent.bottom
        x: xAxisReferenceQuantityLabel._xCentre - xAxisReferenceQuantityLabel._xAdjustment * Math.sign(_guidelineIntersection.x - _indicator.x)
            - width/2
